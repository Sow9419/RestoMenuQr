...
### UC-001 : Création de Compte & Authentification Magic Link
...
* **Flux nominal :**
  ...
  2. Le système envoie un lien OTP unique par e-mail (`supabase.auth.signInWithOtp()`).
  3. L'utilisateur clique sur le lien et sa session d'authentification est initialisée avec succès.
* **Codes d'erreur :** `ERR_MAGIC_LINK_EXPIRED`, `ERR_VALIDATION`

### UC-002 : Création de Restaurant (Tenant)
* **Acteur principal :** Restaurant Owner
* **Préconditions :** Être authentifié.
* **Flux nominal :**
  1. L'Owner renseigne le nom du restaurant et un slug souhaité.
  2. Le système vérifie l'unicité globale du slug.
  3. Le système crée le restaurant et lie le profil de l'utilisateur avec le rôle `OWNER`.
* **Codes d'erreur :** `ERR_SLUG_IMMUTABLE` (si modification ultérieure), `ERR_VALIDATION`
...
* **Flux nominal :**
  ...
  3. Une fois validé, Stripe émet un webhook et le statut de facturation dans `restaurants.subscription_status` se met à jour à `active`.
...
* **Flux nominal :**
  ...
  3. L'employé crée son compte Magic Link et est rattaché avec le rôle ciblé au `restaurant_id` lié de l'invitation dans la table `profiles`.
